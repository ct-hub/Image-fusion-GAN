#******************************************************************************
# Functions for performing transfer learning on the full DL model for image   *
# fusion.                                                                     *
#                                                                             *
...
# Function to perform transfer learning on a pre-trained model.
def transfer_learning(rgb_trans, ir_trans, trans_params):
    # save_model, epochs, check_path, batch_size, learn_rates, spec_norm, sample_img_dir
    # Loading general parameters.
    save_model = trans_params['SAVE']
    epochs = trans_params['EPOCHS']
    main_path = trans_params['MAIN_PATH']
    batch_size = trans_params['BATCH']
    model_id = trans_params['MODEL_ID']
    # Loading learning rates.
    g1_lr = trans_params['GEN1_LR']
    g2_lr = trans_params['GEN2_LR']
    d1_lr = trans_params['DISC1_LR']
    d2_lr = trans_params['DISC2_LR']
    # Loading spectral normalization flags.
    g1_spec = trans_params['GEN1_SPEC']
    g2_spec = trans_params['GEN2_SPEC']
    d1_spec = trans_params['DISC1_SPEC']
    d2_spec = trans_params['DISC2_SPEC']
    # Loading U-Net flag.
    g1_unet_flag = trans_params['GEN1_UNET']
    ...
    gen1_path = main_path + "/Weights/GEN1.h5"
    gen2_path = main_path + "/Weights/GEN2.h5"
    disc1_path = main_path + "/Weights/DISC1.h5"
    disc2_path = main_path + "/Weights/DISC2.h5"
    ...
    sample_img_path = ""
    if(save_model):
        sample_img_path = main_path + "/SamplesTransfer/"
    ...
    for epoch in range(epochs):
        ...
        for batch_rgb, batch_ir in zip(rgb_trans, ir_trans):
            ...
            # With the (not), we optimize the discriminator 1 per every 2
            # generator runs. Without the (not), the inverse happens.
            if(not(batch_count%3 == 0)):
    ...
    # models if required.
    if(save_model):
        # Creating directory with this timestamp.
        dir_path = main_path + "/WeightsTransfer/"
